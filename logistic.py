...
class LightGBM:
    def __init__(self, stats_, games_num):
        ...
        # 天候の要素変換は要チェック
        # self.stats['WorL'].mask(self.stats['WorL'] != 0, 1, inplace=True)
        ...
        for k in range(41):                 # 2022年の試合数
            ...
            if Hteam_stats.loc[game_start, 'HorA'] == 1:        # ホームでの試合の場合，以前の試合を学習
                ...
                if hgame == 1:
                    ...
                    h_study.optimize(h_lgbm, n_trials=100)
                a_study = optuna.create_study(direction='maximize')
                a_study.optimize(a_lgbm, n_trials=100)

                # ベストパラメータを出力
                print('H_params:', h_study.best_params)
                print('A_params:', a_study.best_params)

                H_pre, H_rslt = h_lgbm.Logistic(
                    h_study.best_params['solver'], h_study.best_params['C'], h_study.best_params['max_iter'])
                A_pre, A_rslt = a_lgbm.Logistic(
                    a_study.best_params['solver'], a_study.best_params['C'], a_study.best_params['max_iter'])
